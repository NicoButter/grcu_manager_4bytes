...
class RolSesion {
    ...
    public function setPermisos() {
        ...
        if (!$this->datos) {
            // Se modifica la referencia a ObjetoDatos por DBConexion
            throw new Exception(/*ObjetoDatos*/BDConexion::getInstancia()->errno, BDConexion /*ObjetoDatos*/::getInstancia()->error);
        }
...
class ControlAcceso {
    ...
    function __construct() {
       ...
       /**
        ...
        /*  Acá se excluye explícitamente el index.php de la verificación, o sea, el constructor ya entiende que 
        /  “esta es la página de login, no necesito obligar a verificar si el usuario ya está logueado”.
             Eso evita que el login se redirija a sí mismo indefinidamente.
        */

        if ($this->ubicacion != Constantes::HOMEURL && 
            $this->ubicacion != Constantes::SERVER . '/app/index.php') {
            unset($_SESSION["HTTP_REFERER"]);
            self::verificaLogin();
        } else {
            $_SESSION["HTTP_REFERER"] = Constantes::HOMEURL;
        ...
        /*  Confirmar qué clase realmente se usa (UsuarioSesion o UsuarioGoogle).
            Según la traza que hago -> creaSesion(), hoy es UsuarioSesion.
        */    
        if (
            ($this->ubicacion == Constantes::HOMEURL) &&
            (isset($_SESSION['usuario'])) &&
            ($_SESSION['usuario'] instanceof UsuarioSesion)
        ) {
            $this->redireccionaIndex();
        }
    }
